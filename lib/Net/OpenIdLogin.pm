...
use Net::OpenIdLogin::Types;
use Net::OpenIdLogin::Discover;
use Net::OpenIdLogin::URI;
...
has ua  => (
    ...
    default => sub {
        return LWP::UserAgent->new(
            agent => sprintf 'Net-OpenIdLogin/%s ', __PACKAGE__->VERSION,
            timeout      => $TIMEOUT,
            max_redirect => 0,
        );
    },
...
sub get_openid_endpoint {
    ...
    if (!$endpoint) {
        my $discoverer = Net::OpenIdLogin::Discover->new( ua => $self->ua, claimed_id => $identifier );
...
sub normalize {
    ...
    $uri = Net::OpenIdLogin::URI->normalize($uri);
    return unless Net::OpenIdLogin::URI->is_uri($uri);
...
sub get_extension {
    ...
    unless($extension) {
        $extension = Net::OpenIdLogin::Extension->new(uri => $uri, cgi => $self->cgi, cgi_params => $self->cgi_params);
...
Sending user to be authenticated:
  ...
  my $claimed = 'https://example.openssl.com';
  # this is your OpenID identifier
  
  my $o = Net::OpenIdLogin->new(claimed_id => $claimed, return_to => 'https://example.com/auth');
  my $auth_url = $o->get_auth_url();

Verifying the user was correctly authenticated:

  my $o = Net::OpenIdLogin->new(cgi => $cgi, return_to => 'https://example.com/auth');
  my $id = $o->verify_auth();
...
Hashref of L<Net::OpenIdLogin::Extension> objects (keyed off the extension type URI).
...
Retrieve a single L<Net::OpenIdLogin::Extension> object, based on the type URI provided.
